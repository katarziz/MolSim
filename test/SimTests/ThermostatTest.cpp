...
{ BasicParticleContainer particles=BasicParticleContainer();
  ...
  double temp=thermostat.calculateTemp(particles,Thermostat::calculateAverageVel(particles));
  EXPECT_LE(abs(temp-temp_init),0.5);
}

TEST(TempNonZeroVel, ThermostatTest)
{ BasicParticleContainer particles=BasicParticleContainer();
  Thermostat thermostat=Thermostat();
  thermostat.setParams( 0,0,3);
  double temp_init=40;
  double f_i=std::sqrt(temp_init/1);
  ParticleGenerator::generateCube(particles,{0,0,0},{30,30,30},1,1,5,1,{20,0,0},0,3,f_i);
  std::array<double,3> vel_avg=Thermostat::calculateAverageVel(particles);
  std::cout<<vel_avg<<std::endl;
  double temp=thermostat.calculateTemp(particles,Thermostat::calculateAverageVel(particles));
  EXPECT_LE(abs(temp-temp_init),0.5);
}
...
TEST(ImmediateScalingTest,ThermostatTests)
{
  ...
  std::array<double,3> vel_avg=Thermostat::calculateAverageVel(particles);
  double temp=thermostat.calculateTemp(particles,vel_avg);
  thermostat.scaleV(&particles);
  temp=thermostat.calculateTemp(particles,vel_avg);
...
TEST(MaintainingTest,ThermostatTests)
{
  ...
  double temp_test=thermostat.calculateTemp(particles,vel_avg);
  thermostat.scaleV(&particles);
  temp=thermostat.calculateTemp(particles,vel_avg);
